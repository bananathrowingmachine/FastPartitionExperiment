...
To run the program use python3 on FastPartitionExperiment.py. The current set of python dependencies are 'numpy', 'pandas', 'matplotlib', 'python-docx', 'xlsxwriter', 'cffi' and 'setuptools'. Use ```pip install numpy pandas matplotlib python-docx xlsxwriter cffi setuptools openpyxl``` to install them quickly in a virtual environment.
