...
class ComplexityExperiment:
    ...
    @classmethod
    def testProblemSize(cls, size: int, inputArgs, dataFrame: pd.DataFrame | None = None) -> tuple[np.ndarray, list[DisagreeData]]:
        """
        ...
        :param dataFrame: The data frame generated by pandas for use by the sample output generator. Not used if running the actual experiment.
        ...
        for targetIndex in range(21):
            r = experiment.generateSampleOutput(targetIndex, dataFrame) if inputArgs.example else experiment.runSingleSize(targetIndex)
    ...
    def generateSampleOutput(self, targetIndex: int, dataFrame: pd.DataFrame | None) -> tuple[np.float64, np.float64, np.float64, np.float64, np.float64]:
        """
        Downloads the relevant data off the internet

        :param targetIndex: The target index to get the example sum from.
        :param dataFrame: The URL string of the sheet to attempt to be parsed. If the value is None, the fallback of randomly generated data will be used.
