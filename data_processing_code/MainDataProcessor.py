...
class DataProcessor:
    ...
    def __init__(self, genFilesDir: Path):
        ...
        """
        self.graphDir = genFilesDir / "graphs"
        self.tableDir = genFilesDir / "data tables"
    ...
    def processData(cls, genFilesDir: Path, results: ResultsWrapper):
        ...
        with open(dirs.tableDir / f"testedSetSize{results.IntCount}.txt", "w") as f:
            ...
            for i in range(21):
                ...
                f.write(
                    f"{i}, {row['targetSum']}, {row[1]}, {row['memoNormal']}, {row[3]}, {row['recurseNormal'] if row[4] != np.nan else results.RecurseEstimate}\n")
                
                # Additionally, the logic for recursive normal is rather easy by using results.RecurseEstimate. It will either be a number or np.nan, and will always be the opposite of the results for Recursive Normal.
                # Why is that? Well first, results.RecurseEstimate == np.nan is very simply logic that tells you if Recursive Normal was run. Take a look below for a good example.
                # Second, while Recursive Normal is quite slow, it's by far the most predictable, so once it gets too high, results.RecurseEstimate will become a estimated value that you can sub in for np.nan results, as shown above.
                ...
                if results.RecurseEstimate == np.nan:
